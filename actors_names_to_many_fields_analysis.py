#!/usr/bin/env python3
"""
actors_names_one_to_may_fields_analysis.py

Goal
----
For each JSON produced in `data/actors_matched_names/` (one per agent role),
...
-----
  python actors_names_to_many_fields_analysis.py \
    ...
    --out-dir data/fields_to_values_average_name
"""
...
def main() -> int:
    ...
    ap.add_argument("--in-dir", type=Path, default=Path("data/actors_matched_names"), help="Input folder with actors_*.json files.")
    ap.add_argument("--out-dir", type=Path, default=Path("data/fields_to_values_average_name"), help="Output folder for summaries.")
    args = ap.parse_args()
