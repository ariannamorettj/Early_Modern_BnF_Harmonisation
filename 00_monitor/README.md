...
If a previous run is detected via `resume_info.log`, the header also includes a **Previous run detected** block with the script name, report path, start time, last checkpoint, status, and end time of the previous run.
...
The R monitor maintains a file named `resume_info.log` in the current working directory. This file records the state of the most recent monitoring run and is used to link interrupted runs to their successors.

### File format

`resume_info.log` is a plain key=value text file. Example:

```
script=query_editions
report=00_monitor/report/query_editions_20260713_142305_R.txt
started=2026-07-13 14:23:05
status=INTERRUPTED
last_context=Completed acquisition for year 1550
ended=2026-07-13 16:45:22
```
...
| Field | Description |
|---|---|
| `script` | Name of the entry script that launched the run |
| `report` | Path to the report file generated by that run |
| `started` | Timestamp when the run started |
| `status` | One of `RUNNING`, `COMPLETED`, `INTERRUPTED` |
| `last_context` | Last checkpoint context successfully written before stopping |
| `ended` | Timestamp when the run ended (absent for brutal interruptions) |

### Status values

- **`RUNNING`** — written at the start of every run. If a new run finds `status=RUNNING`, it means the previous run was brutally interrupted (SIGKILL, power loss, crash) — `on.exit()` never ran and the log was never finalised.
- **`COMPLETED`** — written by `stop_monitor_state()` when called explicitly at the end of a normal run.
- **`INTERRUPTED`** — written by `stop_monitor_state()` when called from an `on.exit()` hook triggered by a clean R error or Ctrl+C.

### How it is used

**At run start** — `start_monitor_state()` calls `read_resume_log()`, which:

1. reads and parses `resume_info.log`;
2. prints the previous run summary to console under `[RESUME]`;
3. includes the previous run information in the header of the **new** report file;
4. detects brutal interruption when `status=RUNNING`.

Then `start_monitor_state()` immediately overwrites `resume_info.log` with `status=RUNNING` for the new run.

**At each checkpoint** — `update_monitor_state()` updates `last_context` in `resume_info.log` with the context string of the current checkpoint. This ensures that even a brutal interruption leaves a record of the last completed step.

**At run end** — `stop_monitor_state(status=)` writes the final `status` and `ended` timestamp. The `status` parameter distinguishes normal completion from clean interruption:

- `stop_monitor_state(status = "COMPLETED")` — default, used at the end of a normal run;
- `stop_monitor_state(status = "INTERRUPTED")` — used inside `on.exit()` hooks.

### What the new report header contains when a previous run is detected
...
Previous run detected:
  ...
  Report:          00_monitor/report/query_editions_20260713_142305_R.txt
  Started:         2026-07-13 14:23:05
  Last checkpoint: Completed acquisition for year 1550
  Status:          INTERRUPTED
  Ended:           2026-07-13 16:45:22
```

If the previous run was brutally interrupted, the status line reads:

```
  Status:          BRUTALLY INTERRUPTED (no clean exit detected)
```

and no `Ended` line is printed, since `on.exit()` never ran.

### Functions

#### `parse_resume_log()`

Reads and parses `resume_info.log` into a named list. Returns `NULL` if the file does not exist or is empty.

#### `write_resume_log(script, report_path, started, status, last_context, ended)`

Writes or overwrites `resume_info.log` with the current run state. Called at run start (status=RUNNING), after each checkpoint with a context, and at run end.

#### `read_resume_log()`

Calls `parse_resume_log()`, prints the previous run summary to console, and returns the parsed list invisibly for use by `start_monitor_state()`.
...
# normal completion
state <- stop_monitor_state(state, status = "COMPLETED")
```

When used inside `on.exit()` to handle clean interruptions:

```r
on.exit({
  if (!is.null(state) && !isTRUE(state$closed)) {
    stop_monitor_state(state, status = "INTERRUPTED")
  }
}, add = TRUE)
```
